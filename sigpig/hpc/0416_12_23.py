...
for clustering_Algo in clustering_Algos:
    ...
    #     # create optics model and fit it
    #     clust = OPTICS(min_samples=OPTICS_Min_Samples,
    #                    max_eps=OPTICS_Max_Eps, xi=OPTICS_Xi,
    #                    min_cluster_size=OPTICS_Min_Cluster_Size).fit(
    #                    data_To_Cluster)
    #
    #     # for DBSCAN_Eps_1
    ...
    #                                      eps=DBSCAN_Eps_2)
    #
    #     space = np.arange(len(data_To_Cluster))
    #     reachability = clust.reachability_[clust.ordering_]
    #     labels = clust.labels_[clust.ordering_]
    #
    #     plt.figure(random.randint(0,999), figsize=(10, 7))
    ...
    #
    #     # find number of labels to auto-generate colors
    #     number_Of_Labels = np.ptp(labels)
    #     print(f"Number of OPTICS clusters: {number_Of_Labels}")
    #     # pick a colormap and set the color cycle
    #     cm = plt.get_cmap('tab10')
    #     cNorm = colors.Normalize(vmin=0, vmax= number_Of_Labels- 1)
    #     scalarMap = mplcm.ScalarMappable(norm=cNorm, cmap=cm)
    #     # set axis color cycle for reachability and optics plots
    #     ax1.set_prop_cycle(color=[scalarMap.to_rgba(i) for i in range(
    #                       number_Of_Labels)])
    #     ax2.set_prop_cycle(color=[scalarMap.to_rgba(i) for i in range(
    #         number_Of_Labels)])
    #
    #     # Reachability plot
    ...
    #     ax1.set_title(f'Reachability Plot Params-> Time Scaled by: '
    #                   f'{scale_Factor}; Max Epsilon:'
    #                   f' {OPTICS_Max_Eps} ; Min Clust Size:'
    #               f' {OPTICS_Min_Cluster_Size} ; Xi: {OPTICS_Xi} ; Min Samples:'
    #               f' {OPTICS_Min_Samples} ; PCA: {PCA} : '
    #               f'{PCA_Components if PCA else ""} ; Threshold: {threshold} ; '
    #               f'Number of clusters: {number_Of_Labels}', fontsize=9)
    #
    #     # OPTICS plot
    #     for label in range(0, number_Of_Labels):
    #         Xk = data_To_Cluster[clust.labels_ == label]
    #         ax2.plot(Xk[:, 0], Xk[:, 1], '.')
    #     ax2.plot(data_To_Cluster[clust.labels_ == -1, 0], data_To_Cluster[
    #              clust.labels_ == -1, 1], 'k+', alpha=0.1)
    #     ax2.set_title('Automatic Clustering\nOPTICS')
    #
    #     # DBSCAN at epsilon=DBSCAN_Eps_1
    #     # find number of labels to generate colors
    #     number_Of_Labels = np.ptp(labels_1)
    #     cm = plt.get_cmap('tab10')
    #     cNorm = colors.Normalize(vmin=0, vmax=number_Of_Labels - 1)
    #     scalarMap = mplcm.ScalarMappable(norm=cNorm, cmap=cm)
    #     ax3.set_prop_cycle(color=[scalarMap.to_rgba(i) for i in range(
    #         number_Of_Labels)])
    #     # plot them
    #     for label in range(0, number_Of_Labels):
    #         Xk = data_To_Cluster[labels_1 == label]
    #         ax3.scatter(Xk[:, 0], Xk[:, 1], marker='.')
    #     ax3.plot(data_To_Cluster[labels_1 == -1, 0], data_To_Cluster[
    #              labels_1 == -1, 1], 'k+', alpha=0.1)
    #     ax3.set_title(f'Clustering at {DBSCAN_Eps_1} epsilon\nDBSCAN')
    #
    #     # DBSCAN at epsilon=DBSCAN_Eps_2
    #     # find number of labels to generate colors
    #     number_Of_Labels = np.ptp(labels_2)
    #     cm = plt.get_cmap('tab10')
    #     cNorm = colors.Normalize(vmin=0, vmax=number_Of_Labels - 1)
    #     scalarMap = mplcm.ScalarMappable(norm=cNorm, cmap=cm)
    #     ax4.set_prop_cycle(color=[scalarMap.to_rgba(i) for i in range(
    #         number_Of_Labels)])
    #     # plot them
    #     for label in range(0, number_Of_Labels):
    #         Xk = data_To_Cluster[labels_2 == label]
    #         ax4.scatter(Xk[:, 0], Xk[:, 1], marker='.')
    #     ax4.plot(data_To_Cluster[labels_2 == -1, 0], data_To_Cluster[
    #         labels_2 == -1, 1], 'k+', alpha=0.1)
    #     ax4.set_title(f'Clustering at {DBSCAN_Eps_2} epsilon\nDBSCAN')
    #
    #     plt.tight_layout()
    #     plt.savefig(f'./figures/reachability-{scale_Factor}-{OPTICS_Max_Eps}'
    #                 f'-{OPTICS_Min_Cluster_Size}'
    #                 f'-{OPTICS_Xi}-{OPTICS_Min_Samples}-{PCA}-{PCA_Components if PCA else ""}-{threshold}-{number_Of_Labels}.pdf',
    #                 dpi=300)
    #     #plt.show()
    #
    #      # # # # # # # ###### # # # # # # #
    #     # # # #  N E X T    P L O T  # # # #
    #     # plot clusters by station and time
    ...
    #     figureHeight = 10
    #
    #     plt.figure(random.randint(0,999))
    ...
    #     cNorm = colors.Normalize(vmin=0, vmax=number_Of_Labels - 1)
    #     scalarMap = mplcm.ScalarMappable(norm=cNorm, cmap=cm)
    #     ax.set_prop_cycle(color=[scalarMap.to_rgba(i) for i in range(
    #         number_Of_Labels)])
    #
    #     pick_Station_Order = np.array(pick_Station_Order)
    #     pick_Times = np.array(pick_Times)
    ...
    #         # set marker size to distinguish noise
    #         marker_Size = 9
    #
    #         if k == -1:
    #             # Small markers used for noise
    #             marker_Size = 2
    #
    #         class_member_mask = (labels == k)
    #
    #         # plot in station order vs. time space
    #         y = pick_Station_Order[class_member_mask]
    #         x = pick_Times[class_member_mask]
    #         plt.plot_date(x, y, 'o', markeredgecolor='k', markersize=marker_Size)
    #
    #     plt.title(f'Max Epsilon: {OPTICS_Max_Eps} ; Time Scaled by '
    #               f'{scale_Factor} ; Min Clust Size:'
    #               f' {OPTICS_Min_Cluster_Size} ; Xi: {OPTICS_Xi} ; Min Samples:'
    #               f' {OPTICS_Min_Samples} ; PCA: {PCA} : '
    #               f'{PCA_Components if PCA else ""} ; Threshold: {threshold} ; '
    #               f'Number of clusters: {n_clusters_}')
    #
    #     ax.set_xlabel('Time (HH:MM:SS) of specified day')
    #     ax.set_yticks(np.arange(0, len(station_Distance)))
    #     ax.set_yticklabels(station_Distance.keys())
    #     ax.set_ylabel('Station')
    ...
    #     plt.savefig(f'./figures/clusters-{scale_Factor}-{OPTICS_Max_Eps}'
    #                 f'-{OPTICS_Min_Cluster_Size}'
    #                 f'-{OPTICS_Xi}-{OPTICS_Min_Samples}-{PCA}-{PCA_Components if PCA else ""}-{threshold}-{n_clusters_}.pdf',
    #                 dpi=300)
    #     plt.show()
