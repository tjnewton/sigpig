...
def plot_Time_Series_And_Spectrogram(doi, doi_end, files_path, filter=False, bandpass=[]):
	"""
	Plots time series and spectrograms for all files in the `files_path` for the specified time period starting at `doi` and ending at `doi_end`. Data are first bandpass filtered if `filter=True`.
	...
	for index, trace in enumerate(st):
	    ...
	    spec.pcolormesh(tSTFT, fSTFT, 20 * np.log10(np.absolute(STFT)))  # dB = 20*log() convention
	    spec.set_xlim([0, duration - window_length / trace.stats.sampling_rate])
	    spec.set_ylabel(f"{trace.stats.network}.{trace.stats.station}.{trace.stats.channel}",
