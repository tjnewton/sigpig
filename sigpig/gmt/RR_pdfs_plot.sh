...
gmt set FORMAT_GEO_OUT=-D FORMAT_GEO_MAP=-D FORMAT_FLOAT_OUT=%.12g
...
gmt begin events_$VEL_RANGE png
	...
	gmt basemap -Rk694.150000/694.448000/5155.400000/5155.898000 -JU+10/? -Baf -BWeNs+t"Event PDF Locations n=$NUM_EVENTS Vp=$VEL_RANGE"
	...
	# to go from weighted event location pdf points to a heatmap: count weights with blockmean,
	# then use surface to convert to grid, grdfilter to trim, and grdview to plot
	...
	gmt makecpt -Cmag -T0/50000 # -H >> pdfs.cpt
	# plot the grid
	gmt grdview gridded_pdf_$VEL_RANGE.nc -C -Qsm -t20
	# trim and plot the colorbar
	gmt colorbar -C -DJMR+o0.8c/0+w10c/0.3c -B50000 -Bx+l"Weight sum" -G0/50000


	# plot events as stars
	# gmt plot xyw_$VEL_RANGE.csv -Gblack -Sa0.4 -W0.1,black
